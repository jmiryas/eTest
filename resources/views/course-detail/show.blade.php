<x-layout.app title="Detail Peserta Kursus" activeMenu="course-detail.show" :withError="true">
    <div class="container my-5">
        <x-breadcrumb title="Detail Peserta Kursus" :breadcrumbs="[
            ['label' => 'Dashboard', 'url' => url('/')],
            ['label' => 'Peserta Kursus', 'url' => route('course-detail.index')],
            ['label' => 'Detail Peserta Kursus'],
        ]" />
                ...
                <div class="d-flex justify-content-between">
                    ...
                    <div>
                        @can('etest course-detail view')
                            <a href="{{ route('course-detail.create') }}" class="btn btn-sm btn-info">
                                <i class="bx bx-plus me-1"></i>Baru
                            </a>
                        @endcan
                        @can('etest course-detail edit')
                            <a href="{{ route('course-detail.edit', $courseDetail) }}" class="btn btn-sm btn-primary">
                                <i class="bx bx-pencil me-1"></i>Edit
                            </a>
                        @endcan
                        @can('etest course-detail delete')
                            <form action="{{ route('course-detail.destroy', $courseDetail) }}" method="POST"
                                class="d-inline">
                                @csrf @method('DELETE')
                                <x-input.confirm-button text="Data course detail ini akan dihapus!" positive="Ya, hapus!"
