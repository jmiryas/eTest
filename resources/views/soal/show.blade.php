<x-layout.app title="Detail Paket Soal" activeMenu="soal.show" :withError="true">
    <div class="container my-5">
        <x-breadcrumb title="Detail Paket Soal" :breadcrumbs="[
        ...
        <div class="card">
            <div class="card-header">
                <div class="d-flex justify-content-between">
                    ...
                    <div>
                        @can('etest soal view')
                            <a href="{{ route('soal.create') }}" class="btn btn-sm btn-info">
                                <i class="bx bx-plus me-1"></i>Baru
                            </a>
                        @endcan
                        @can('etest soal edit')
                            <a href="{{ route('soal.edit', $soal) }}" class="btn btn-sm btn-primary">
                                <i class="bx bx-pencil me-1"></i>Edit
                            </a>
                        @endcan
                        @can('etest soal delete')
                            <form action="{{ route('soal.destroy', $soal) }}" method="POST" class="d-inline">
                                @csrf @method('DELETE')
